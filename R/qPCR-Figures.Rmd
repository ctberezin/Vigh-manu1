...
```{r}
#not all samples but properly calibrated
pomc_gs <- read.csv("../data/01202022-POMC-circ-ld-gene-study-INCOMPLETE.csv",
                     fileEncoding = 'UTF-8-BOM')
pomc_gs <- pomc_gs %>% mutate(Sex = as.factor(Sex),
                                Hour = as.factor(Hour),
                                Time = as.factor(Time),
                                Light = as.factor(Light))
pomc_gs <- pomc_gs %>% mutate(Hour = fct_relevel(Hour, c("8am", "12pm", "4pm", "12am", "1am")))

ggplot(pomc_gs, aes(x=Hour, y=RelativeGE)) +
  geom_point(aes(col=Light)) +
  theme_few() +
  stat_summary(fun = 'mean', geom="bar", aes(alpha=0.5))

shapiro.test(pomc_gs$RelativeGE)
hist(pomc_gs$RelativeGE)

pomc_gs_lm <- lm(RelativeGE ~ Sex*Light*Time, pomc_gs)

qqnorm(pomc_gs_lm$residuals)
qqline(pomc_gs_lm$residuals)

#all samples but not properly calibrated
pomc_all <- read.csv("../data/01202022-POMC-circ-ld-gene-study-NO-CALIB.csv",
                     fileEncoding = 'UTF-8-BOM')
...
ggplot(pomc_all, aes(x=Hour, y=RelativeGE)) +
  geom_point(aes(col=Light)) +
  theme_few() +
  facet_wrap(~Sex)

shapiro.test(pomc_all$RelativeGE)
```
