...
SumStats_phase_grps <- SumStats_phase %>% group_by(Genotype, Sleep.Stage, phase) %>% 
  summarise(n = n(),
            avg_bout_length = mean(mean_bout_length),
            min_bout_length = min(min_bout_length),
            max_bout_length = max(max_bout_length),
            sd_bout_length = sd(mean_bout_length))
SumStats_phase_grps
...
hist(SumStats_phase$mean_bout_length, breaks=50)
hist(resid(lm_mean_length), breaks=50)
anova(lm_mean_length)
emmeans(lm_mean_length, pairwise ~ Genotype | Sleep.Stage * phase)$contrasts

lm_log_mean_length <- lmer(log(mean_bout_length) ~ Sleep.Stage * phase * Genotype + (1 | Transmitter), data=SumStats_phase)
plot(lm_log_mean_length, type=c("p","smooth"), col.line=1)
lattice::qqmath(lm_log_mean_length)
anova(lm_mean_length)
emmeans(lm_mean_length, pairwise ~ Genotype | Sleep.Stage * phase)$contrasts
...
lm_duration <- lmer(total_duration ~ Sleep.Stage * phase * Genotype + (1 | Transmitter), data=SumStats_phase)
...
```{r}
sws_bouts <- SumStats_phase %>% filter(Sleep.Stage == "S" & phase == "light") %>% 
  ggplot(aes(x=Genotype, y=mean_bout_length)) +
  stat_summary(fun = 'mean', geom="bar", width=0.75) +
  stat_summary(fun.data = mean_se, geom = "errorbar", width=.2) +
  geom_point() +
  ylab("Average SWS bout length in the light phase (s)") +
  theme_bw() +
  scale_y_continuous(expand = expansion(mult = c(0, 0.2)))

sws_bouts
#ggsave(filename="../figures/sws_bouts.svg", plot=sws_bouts, height=4, width=3)
```
...
length_phase_plot <- SumStats_phase %>% filter(Sleep.Stage != "Artifact") %>% 
  ggplot(aes(x=phase, y=mean_bout_length)) +
...
daily_duration_plot <- SumStats_phase %>% filter(Sleep.Stage != "Artifact") %>% 
  ggplot(aes(x=phase, y=total_duration)) +
  facet_wrap(~Sleep.Stage, scales="free_y") +
  geom_boxplot(aes(color=Genotype)) +
  ylab("Total daily duration (s)") +
  theme_bw() +
  scale_y_continuous(expand = expansion(mult = c(0, 0.2)))
