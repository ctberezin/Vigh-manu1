...
```{r}
#1 hour bins (by ZT)
xl <- read.csv("../data/McKO_WT_combined_sleep.csv")
...
wake_plot <- ggplot(data=SumStatwt, aes(ZT, mean_Wake, color=Genotype)) +
  ...
  geom_errorbar(data=SumStatwt, aes(x= ZT, ymin=mean_Wake-se_Wake,
                                    ymax=mean_Wake+se_Wake, fill=Genotype),
                width=.5) +
...
#ggsave("../figures/wake_plot.png", plot=wake_plot, height=5, width=9)
