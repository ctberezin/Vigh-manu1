...
temp %>% group_by(Genotype) %>% 
  summarise(n = n(),
            ...
            median_wake = median(mean_wake),
            sws_perc = mean(mean_sws),
            sd_sws = sd(mean_sws),
            median_sws = median(mean_sws),
            ps_perc = mean(mean_ps),
            sd_ps = sd(mean_ps),
            median_ps = median(mean_ps))
...
sws_day
sws_Zstat <- qnorm(sws_day$p.value/2)
abs(sws_Zstat)/sqrt(9.5)

ps_day <- wilcox.test(mean_ps ~ Genotype, data=t_wt_days)
ps_day
ps_Zstat <- qnorm(ps_day$p.value/2)
abs(ps_Zstat)/sqrt(9.5)
