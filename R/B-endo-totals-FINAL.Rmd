...
counts <- counts %>% mutate(group = as.factor(c(1,1,1,4,4,4,2,2,2,3,3,3)))
counts

levels(counts$group)

gcl_group_lm <- lm(GCL_bendo_mm ~ group, data=counts)
ScheffeTest(aov(gcl_group_lm), contrasts=c(1, -1/3, -1/3, -1/3))
ScheffeTest(aov(gcl_group_lm), contrasts=c(1, 0, -1/2, -1/2))
inl_group_lm <- lm(INL_bendo_mm ~ group, data=counts)
ScheffeTest(aov(inl_group_lm), contrasts=c(1, -1/3, -1/3, -1/3))
ScheffeTest(aov(inl_group_lm), contrasts=c(1, 0, -1/2, -1/2))
```
...
emmeans::emmeans(gcl_bendo_lm, pairwise ~ light | time)
emmeans::emmeans(gcl_bendo_lm, pairwise ~ time | light)

emmeans::emmeans(inl_bendo_lm, pairwise ~ light | time)
emmeans::emmeans(inl_bendo_lm, pairwise ~ time | light)
...
gcl_inter_plot <- ggplot() +
  ...
  #geom_errorbar(data=counts_sumstats, aes(x=time, ymin=mean_GCL_bendo_mm-sd_GCL_bendo_mm,
  #              ymax=mean_GCL_bendo_mm+sd_GCL_bendo_mm), width=0.1) +
...
inl_inter_plot <- ggplot() +
  ...
  stat_summary(data=counts, aes(x = time, color = light, group = light, y = INL_bendo_mm),
               fun = mean, geom = "line", size=1) +
  #geom_errorbar(data=counts_sumstats, aes(x=time, ymin=mean_INL_bendo_mm-sd_INL_bendo_mm,
  #              ymax=mean_INL_bendo_mm+sd_INL_bendo_mm), width=0.1) +
