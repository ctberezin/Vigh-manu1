...
```{r}
pomc_ld <- read.csv("../data/pomc-ld-m.csv",
                      fileEncoding = 'UTF-8-BOM')
pomc_ld <- pomc_ld %>% mutate(Sample = as.factor(Sample),
                              Time = as.factor(Time),
                              Light = fct_relevel(as.factor(Light), c("light", "dark")),
                              Run = as.factor(Run))

pomc_ld_summ <- pomc_ld %>% group_by(Time, Light) %>%
                    summarise(n = n(),
                              mean = mean(RGE),
                              sd = sd(RGE))
...
pomc_ld_plot <- ggplot() +
  stat_summary(data=pomc_ld, aes(x=Time, y=RGE, fill=Light), fun = 'mean', geom="bar",
               width=0.75, position="dodge") +
  geom_errorbar(data=pomc_ld_summ, aes(x=Time, ymin=mean-sd, ymax=mean+sd, fill=Light),
                width=.2, position=position_dodge(0.75)) +
  geom_point(data=pomc_ld, aes(x=Time, y=RGE, fill=Light), size = 2, position=position_dodge(0.75)) +
  theme_bw() %+replace%
  theme(plot.background = element_rect(fill = "transparent",colour = NA),
        panel.background = element_rect(fill = "transparent",colour = NA)) +
  labs(x="Zeitgeber Time (ZT)",
...
#ggsave(filename="../figures/pomc_ld.png", plot=pomc_ld_plot, height=4, width=4)
```

```{r}
pomc_ld_lm <- lm(log(RGE) ~ Time * Light, pomc_ld)
plot(pomc_ld_lm, which=c(1,2))

car::Anova(pomc_ld_lm, type="3")
emmeans(pomc_ld_lm, pairwise ~ Time | Light)$contrasts
emmeans(pomc_ld_lm, pairwise ~ Light | Time)$contrasts

pomc_ld_summ <- pomc_ld %>% group_by(Time, Light) %>%
                    summarise(log_mean = mean(log(RGE)),
                              log_sd = sd(log(RGE)))

pomc_ld_summ

pomc_ld_log_plot <- ggplot() +
  stat_summary(data=pomc_ld, aes(x=Time, y=log(RGE), fill=Light), fun = 'mean', geom="bar",
               width=0.75, position="dodge") +
  geom_errorbar(data=pomc_ld_summ, aes(x=Time, ymin=log_mean-log_sd, ymax=log_mean+log_sd, fill=Light),
                width=.2, position=position_dodge(0.75)) +
  geom_point(data=pomc_ld, aes(x=Time, y=log(RGE), fill=Light), size = 2, position=position_dodge(0.75)) +
  theme_bw() %+replace%
  theme(plot.background = element_rect(fill = "transparent",colour = NA),
        panel.background = element_rect(fill = "transparent",colour = NA)) +
  labs(x="Zeitgeber Time (ZT)",
...
#ggsave(filename="../figures/pomc_ld_log.png", plot=pomc_ld_log_plot, height=4, width=4)
```
...
#ggsave(filename="../figures/pomc_hyp_ret.svg", plot=pomc_hyp_plot, height=4, width=4)
...
emmeans(pomc_hyp_log_lmer, pairwise ~ Tissue)$contrasts

pomc_hyp_log_plot <- ggplot() +
  stat_summary(data=pomc_hyp, aes(x=Condition, y=log(RGE), fill=Condition),
               fun = 'mean', geom="bar", width=0.75) +
  geom_errorbar(data=pomc_hyp_summ, aes(x=Condition, ymin=log_mean-log_sd, ymax=log_mean+log_sd), width=0.2) +
...
#ggsave(filename="../figures/pomc_hyp_ret_log.png", plot=pomc_hyp_log_plot, height=4, width=4)
```

```{r}
pomc_hyp <- pomc_hyp %>% mutate(Tissue = fct_relevel(Tissue, "retina", "hypothalamus"))

pomc_tissue_plot <- ggplot() +
  stat_summary(data=pomc_hyp, aes(x=Tissue, y=RGE, fill=Condition),
               fun = 'mean', geom="bar", width=0.75) +
  geom_errorbar(data=pomc_hyp_summ, aes(x=Tissue, ymin=mean-sd, ymax=mean+sd), width=0.2) +
  geom_point(data=pomc_hyp, aes(x=Tissue, y=RGE), size=2) +
...
#ggsave(filename="../figures/pomc_tissue.svg", plot=pomc_tissue_plot, height=4, width=4)

pomc_tissue_log_plot <- ggplot() +
  stat_summary(data=pomc_hyp, aes(x=Tissue, y=log(RGE), fill=Condition),
               fun = 'mean', geom="bar", width=0.75) +
  geom_errorbar(data=pomc_hyp_summ, aes(x=Tissue, ymin=log_mean-log_sd, ymax=log_mean+log_sd), width=0.2) +
  geom_point(data=pomc_hyp, aes(x=Tissue, y=log(RGE)), size=2) +
...
pomc_tissue_log_plot

#ggsave(filename="../figures/pomc_tissue_log.png", plot=pomc_tissue_log_plot, height=4, width=4)
```
