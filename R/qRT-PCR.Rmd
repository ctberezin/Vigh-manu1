...
```{r}
#bad
mor_lm <- lm(RelativeGE ~ Genotype * Tissue, mor)
plot(mor_lm, which=c(1,2))

#better
mor_log_lm <- lm(log(RelativeGE) ~ Genotype * Tissue, mor)
plot(mor_log_lm, which=c(1,2))
#ok
car::leveneTest(log(mor$RelativeGE) ~ Genotype * Tissue, mor)
#still doesn't pass
shapiro.test(log(mor$RelativeGE))



#better off separating the tissues to achieve normality

mor_retina <- mor %>% filter(Tissue == "retina")
mor_hyp <- mor %>% filter(Tissue == "hypothalamus")


#retina analysis
mor_ret_lm <- lm(RelativeGE ~ Genotype, mor_retina)
plot(mor_ret_lm, which=c(1,2))
#better homogenous variance but normality looks same
mor_ret_log_lm <- lm(log(RelativeGE) ~ Genotype, mor_retina)
plot(mor_ret_log_lm, which=c(1,2))

shapiro.test(mor_retina$RelativeGE)
#normality slightly better, both pass
shapiro.test(log(mor_retina$RelativeGE))
car::leveneTest(RelativeGE ~ Genotype, mor_retina)
#homogenous variance definitely better
car::leveneTest(log(RelativeGE) ~ Genotype, mor_retina)

#significantly different in the retina
t.test(log(mor_retina$RelativeGE) ~ mor_retina$Genotype)

mor_retina <- mor_retina %>% mutate(log_rge = log(RelativeGE))

mor_ret_summ <- mor_retina %>% group_by(Genotype) %>% 
      summarise(n = n(),
                log_mean = mean(log_rge),
                log_sd = sd(log_rge))

mor_ret_log_plot <- ggplot() +
  stat_summary(data=mor_retina, aes(x=Genotype, y=log_rge, fill=Genotype),
               fun = 'mean', geom="bar", width=0.75) +
  geom_errorbar(data=mor_ret_summ, aes(x=Genotype, ymin=log_mean-log_sd, ymax=log_mean+log_sd), width=0.2) +
  geom_point(data=mor_retina, aes(x=Genotype, y=log_rge), size=2) +
  theme_bw() %+replace%
  theme(legend.position = "none") +
  labs(y="Relative Gene Expression",
       x="") +
...
#ggsave(filename="../figures/mor_retina_log.svg", plot=mor_ret_log_plot, height=4, width=3)

#hypothalamus analysis
mor_hyp_lm <- lm(RelativeGE ~ Genotype, mor_hyp)
plot(mor_hyp_lm, which=c(1,2))
#not much better on either front, homogenous variance looks good, let's look into normality further
mor_hyp_log_lm <- lm(log(RelativeGE) ~ Genotype, mor_hyp)
plot(mor_hyp_log_lm, which=c(1,2))

#passes just fine
shapiro.test(mor_hyp$RelativeGE)

#as expected, both fine
car::leveneTest(RelativeGE ~ Genotype, mor_hyp)
car::leveneTest(log(RelativeGE) ~ Genotype, mor_hyp)

#not significantly different in hypothalamus
t.test(mor_hyp$RelativeGE ~ mor_hyp$Genotype)
```
